...
import type { Contract, ContractInput, ContractType } from '../../api/types';
import { contractTypeLabels } from '../../i18n';
...
/** Reusable contract fields form. Handles both create and edit; calls onSaved with the result. */
export function ContractForm({ contractId, onSaved, onCancel, submitLabel = 'Lưu' }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (input: ContractInput) =>
      isEdit ? updateContract(contractId as string, input) : createContract(input),
    onSuccess: (saved) => {
      ...
      <form className="form card" onSubmit={submit} noValidate>
        <div className="form-grid">
          ...
          <label>
            ...
            {errors.personInChargeId && <small className="error">{errors.personInChargeId}</small>}
